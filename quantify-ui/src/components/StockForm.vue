...
const isSubmissionAllowed = ref(false)
const emit = defineEmits(['request-model'])
const formValues = [ticker, forecastPeriod, chosenModel, startDate, lookBack, epochs, batchSize]
...
const updateMaxDate = (e) => {
    ...
    .then(res => {
        ...
        // Reset start date if it is greater than the new max start date
...
// If an input field is empty, disable the forecast button
const setIsSubmissionAllowed = () => {
    let isFieldCleared = false
    ...
    isSubmissionAllowed.value = !isFieldCleared
}
...
<template>
    ...
    <form>
        ...
        <div>
            <SelectButton disabled="true" id="modelInput" v-model="chosenModel" :options="modelOptions" @input="setIsSubmissionAllowed"/>
        </div>
        <label for="dateInput">Data Start Date</label>
        <div>
            <DatePicker id="dateInput" :max-date="maxDate" @input="setIsSubmissionAllowed" v-model="startDate"/>
        </div>
        <label for="forecastInput">Forecast Length</label>
        <div>
            <InputNumber id="forecastInput" :min="1" @input="setIsSubmissionAllowed" v-model="forecastPeriod"/> 
        </div>
        <div v-if="chosenModel === 'LSTM'">
            ...
            <div>
                <InputNumber id="epochsInput"  :min="1" :max="5" @input="setIsSubmissionAllowed" v-model="epochs"/>
            </div>
            <label for="batchSizeInput">Batch Size</label>
            <div>
                <InputNumber id="batchSizeInput"  :min="1" :max="30" @input="setIsSubmissionAllowed" v-model="batchSize"/>
            </div>
        </div>
        <PrimeButton class="forecast-button" label="Forecast" :disabled="!isSubmissionAllowed" @click="handleSubmit"/>
